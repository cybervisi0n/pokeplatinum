#ifndef POKEPLATINUM_CONSTANTS_HEAP_H
#define POKEPLATINUM_CONSTANTS_HEAP_H

enum HeapID {
    HEAP_ID_SYSTEM = 0,
    HEAP_ID_SAVE,
    HEAP_ID_DEBUG,
    HEAP_ID_APPLICATION,
    HEAP_ID_FIELD1,
    HEAP_ID_BATTLE,
    HEAP_ID_BAG,
    HEAP_ID_POKETCH_MAIN,
    HEAP_ID_POKETCH_APP,
    HEAP_ID_BOX_DATA,
    HEAP_ID_BOX_GRAPHICS,
    HEAP_ID_FIELD2,
    HEAP_ID_PARTY_MENU,
    HEAP_ID_13,
    HEAP_ID_14,
    HEAP_ID_COMMUNICATION,
    HEAP_ID_16,
    HEAP_ID_TOWN_MAP,
    HEAP_ID_NAMING_SCREEN_APP,
    HEAP_ID_POKEMON_SUMMARY_SCREEN,
    HEAP_ID_20,
    HEAP_ID_21,
    HEAP_ID_22,
    HEAP_ID_23,
    HEAP_ID_24,
    HEAP_ID_TRAINER_CASE,
    HEAP_ID_TRADE_ROOM,
    HEAP_ID_27,
    HEAP_ID_28,
    HEAP_ID_MINING,
    HEAP_ID_DISTORTION_WORLD_WARP,
    HEAP_ID_31,
    HEAP_ID_FIELD3,
    HEAP_ID_UNDERGROUND,
    HEAP_ID_34,
    HEAP_ID_35,
    HEAP_ID_BERRY_TAG,
    HEAP_ID_POKEDEX,
    HEAP_ID_OPTIONS_MENU,
    HEAP_ID_DRAWING,
    HEAP_ID_MAIL,
    HEAP_ID_MAIL_VIEWER,
    HEAP_ID_JOURNAL,
    HEAP_ID_43,
    HEAP_ID_POFFIN_CASE,
    HEAP_ID_POFFIN_MANAGER,
    HEAP_ID_POFFIN_CUTSCENE,
    HEAP_ID_CHOOSE_STARTER_APP,
    HEAP_ID_48,
    HEAP_ID_NINTENDO_WFC,
    HEAP_ID_50,
    HEAP_ID_51,
    HEAP_ID_52,
    HEAP_ID_53,
    HEAP_ID_54,
    HEAP_ID_POFFIN_BERRY_SELECTION,
    HEAP_ID_56,
    HEAP_ID_TRADE_SEQUENCE,
    HEAP_ID_TRADE_SEQUENCE_PHASE,
    HEAP_ID_59,
    HEAP_ID_60,
    HEAP_ID_PC_HALL_OF_FAME,
    HEAP_ID_62,
    HEAP_ID_HALL_OF_FAME,
    HEAP_ID_64,
    HEAP_ID_65,
    HEAP_ID_66,
    HEAP_ID_MOVE_REMINDER,
    HEAP_ID_68,
    HEAP_ID_69,
    HEAP_ID_70,
    HEAP_ID_EGG_HATCH,
    HEAP_ID_BOAT_CUTSCENE,
    HEAP_ID_EVOLUTION,
    HEAP_ID_BATTLE_TOWER_RECORDS_APP,
    HEAP_ID_END_CREDITS,
    HEAP_ID_76,
    HEAP_ID_GAME_START,
    HEAP_ID_MIGRATE_FROM_GBA,
    HEAP_ID_79,
    HEAP_ID_RANKINGS_MACHINE,
    HEAP_ID_MAIN_MENU,
    HEAP_ID_ROWAN_INTRO,
    HEAP_ID_ROWAN_INTRO_TV,
    HEAP_ID_DIPLOMA,
    HEAP_ID_RANGER_LINK,
    HEAP_ID_MYSTERY_GIFT_APP,
    HEAP_ID_WONDER_CARDS_APP,
    HEAP_ID_88,
    HEAP_ID_89,
    HEAP_ID_90,
    HEAP_ID_NETWORK_ICON,
    HEAP_ID_92,
    HEAP_ID_BATTLE_FACTORY_APP,
    HEAP_ID_94,
    HEAP_ID_95,
    HEAP_ID_96,
    HEAP_ID_97,
    HEAP_ID_BATTLE_HALL_APP,
    HEAP_ID_99,
    HEAP_ID_BATTLE_CASTLE_APP,
    HEAP_ID_101,
    HEAP_ID_102,
    HEAP_ID_BATTLE_ARCADE_APP,
    HEAP_ID_104,
    HEAP_ID_105,
    HEAP_ID_106,
    HEAP_ID_107,
    HEAP_ID_108,
    HEAP_ID_109,
    HEAP_ID_110,
    HEAP_ID_111,
    HEAP_ID_112,
    HEAP_ID_113,
    HEAP_ID_FRONTIER_RECORD_APP,
    HEAP_ID_SCRATCH_OFF_CARD_APP,
    HEAP_ID_116,
    HEAP_ID_117,
    HEAP_ID_118,
    HEAP_ID_119,
    HEAP_ID_LIBRARY_TV,
    HEAP_ID_PARTY_MENU1,
    HEAP_ID_PARTY_MENU2,
    HEAP_ID_MAX,
};

enum HeapSize {
#ifdef SDK_BUILD_ARM
    HEAP_SIZE_SYSTEM = 0xD200,
    HEAP_SIZE_SAVE = 0x20E00,
    HEAP_SIZE_DEBUG = 0x10,
    HEAP_SIZE_APPLICATION = 0x10D800,
#else
    HEAP_SIZE_SYSTEM = 0xD200*2,
    HEAP_SIZE_SAVE = 0x20E00*2,
    HEAP_SIZE_DEBUG = 0x10*2,
    HEAP_SIZE_APPLICATION = 0x10D800*2,
#endif

    HEAP_SIZE_BATTLE_HALL_APP = 0x20000,
    HEAP_SIZE_BATTLE_FACTORY_APP = 0x20000,
    HEAP_SIZE_BATTLE_ARCADE_APP = 0x20000,
    HEAP_SIZE_FRONTIER_RECORDS_APP = 0x20000,
    HEAP_SIZE_BAG = 0x30000,

    HEAP_SIZE_PRE_POKETCH_SUBSCREEN = 0x18000,
    #ifdef SDK_PORT
    HEAP_SIZE_POKETCH_MAIN = 0xC000*2,
    HEAP_SIZE_POKETCH_APP = 0xC000*2,
    #else
    HEAP_SIZE_POKETCH_MAIN = 0xC000,
    HEAP_SIZE_POKETCH_APP = 0xC000,
    #endif
    HEAP_SIZE_BOX_DATA = 0x4000,
    HEAP_SIZE_BOX_GRAPHICS = 0x3C000,

    #ifdef SDK_PORT
    HEAP_SIZE_FIELD1 = 0x20000*2,
    HEAP_SIZE_FIELD2 = 0x1C000*2,
    HEAP_SIZE_FIELD3 = 0x4000*2,
    #else
    HEAP_SIZE_FIELD1 = 0x20000,
    HEAP_SIZE_FIELD2 = 0x1C000,
    HEAP_SIZE_FIELD3 = 0x4000,
    #endif

    HEAP_SIZE_TOWN_MAP = 0x20000,

    #ifdef SDK_PORT
    HEAP_SIZE_EGG_HATCH = 0x80000,
    #else
    HEAP_SIZE_EGG_HATCH = 0x40000,
    #endif
    HEAP_SIZE_CHOOSE_STARTER_APP = 0x40000,
    HEAP_SIZE_LIBRARY_TV = 0x40000,
    HEAP_SIZE_TRAINER_CASE = 0x28000,
    HEAP_SIZE_MINING = 0x50000,
    HEAP_SIZE_DISTORTION_WORLD_WARP = 0x50000,
    HEAP_SIZE_UNDERGROUND = 0xE800,
    HEAP_SIZE_DRAWING = 0x40000,
    HEAP_SIZE_MAIL = 0x1000,
    HEAP_SIZE_MAIL_VIEWER = 0x20000,
    HEAP_SIZE_BOAT_CUTSCENE = 0x20000,
    #ifdef SDK_BUILD_ARM
    HEAP_SIZE_EVOLUTION = 0x30000,
    #else
    HEAP_SIZE_EVOLUTION = 0x30000*2,
    #endif
    HEAP_SIZE_GAME_START = 0x20000,
    HEAP_SIZE_RECORD_LIST = 0x6000,
    HEAP_SIZE_MAIN_MENU = 0x40000,
    HEAP_SIZE_MIGRATE_FROM_GBA = 0x38000,
    HEAP_SIZE_RANGER_LINK = 0x20000,
    #ifdef SDK_BUILD_ARM
    HEAP_SIZE_MYSTERY_GIFT_APP = 0x30000,
    #else
    HEAP_SIZE_MYSTERY_GIFT_APP = 0x30000*2,
    #endif
    HEAP_SIZE_WONDER_CARDS_APP = 0x30000,
    HEAP_SIZE_NETWORK_ICON = 0x300,
};

#endif // POKEPLATINUM_CONSTANTS_HEAP_H
